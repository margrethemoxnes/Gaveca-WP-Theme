...
function custom_login_logo() {
	echo '<style type="text/css">
		h1 a { 
			background-image:url('. get_bloginfo( 'template_directory' ) .'/assets/images/gaveca-logo_200x64.png) !important; 
...
function forfatter_taxonomy_edit_meta_field($term) {
	...
	<tr class="form-field">
		...
		<td><input type="text" name="term_meta[custom_term_meta]" id="term_meta[custom_term_meta]" value="<?php echo esc_attr( $term_meta['custom_term_meta'] ) ? esc_attr( $term_meta['custom_term_meta'] ) : ''; ?>"> <p class="description"><?php _e( 'Skriv inn en undertittel','forfatter_undertittel' ); ?></p></td>
	</tr>
<?php
}
	...
	$terms = apply_filters( 'taxonomy-images-get-terms', '', array(
		...
		if ( ! empty( $terms ) ) {
			...
			foreach( (array) $terms as $term ) {
				...
				<a href="' . esc_url( get_term_link( $term, $taxID ) ) . '">' . wp_get_attachment_image( $term->image_id, 'bok') . '</a>'; 
